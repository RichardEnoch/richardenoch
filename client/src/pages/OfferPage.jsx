...
export default function OfferPage() {
  ...
  useEffect(() => {
    ...
    fetchJson(`/api/discounts/offer/${token}`)
      .then((o) => {
        if (on) o?.ok ? setOffer(o) : setError(o?.error || "Offer not found.");
      })
      .catch((e) => {
        if (on) setError(e?.message || "This offer link isn't valid anymore.");
      });
    return () => {
      on = false;
    };
  ...
  return (
    <div className="mx-auto min-h-[70vh] w-full max-w-[640px] px-5 pt-32 pb-20 text-white">
      {!offer && !error && (
        <p className="text-center text-neutral-400">Opening your offer…</p>
      )}
      ...
      {offer && (
        <div className="rounded-2xl border border-lime-400/25 bg-white/[0.02] p-8">
          <div className="mb-1 text-xs uppercase tracking-[0.2em] text-lime-400">
          ...
          <p className="mb-6 text-sm leading-relaxed text-neutral-400">
            {offer.note ||
              "As we discussed, here is your agreed package. Book through this page and your invoice will carry exactly this price."}
          </p>
          ...
          <div className="mb-6 space-y-3">
            {offer.items.map((item) => {
              ...
              return (
                <div
                  key={item.service}
                  className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-4 py-3.5"
                >
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold">
                      {meta.label} —{" "}
                      <span className="capitalize">{item.planKey}</span>
                    </div>
                    <div className="text-xs text-neutral-500">
                      Agreed price:{" "}
                      <span className="text-lime-300">{N(item.price)}</span>
                    </div>
                  </div>
                  {item.booked ? (
