...
const LABELS = {
  ...
  business_about: "About the business",
  focus: "Business focus",
  service_areas: "Service areas",
  trust_signals: "Certifications / trust signals",
  peculiarity: "What makes the firm different",
  goals_short: "Short-term goals",
  goals_long: "Long-term goals",
  audience: "Target audience",
  values: "Brand values & mission",
  perception: "Desired perception",
  aesthetic: "Design aesthetic",
  colors: "Colour preferences",
  existing_logo: "Existing logo",
  inspiration: "Brands admired",
  avoid: "Things to avoid",
  competitors: "Competitors",
  tagline: "Tagline",
  name_story: "Idea behind the name",
  brand_vision: "Vision for the brand & logo",
  personality: "Brand personality",
  merch_designs: "Merch designs picked",
  site_purpose: "Website purpose",
  site_actions: "Visitor actions",
  specialty_page: "Specialty page focus",
  content_status: "Content readiness",
  photos: "Project photos",
  site_refs: "Reference websites",
  features: "Website features",
  domain_1: "Domain — 1st choice",
  domain_2: "Domain — 2nd choice",
  domain_3: "Domain — 3rd choice",
  social_active: "Active social platforms",
  social_platforms: "Audience platforms",
  social_content: "Social designs should promote",
  social_tone: "Tone of voice",
  social_refs: "Admired accounts",
  duration: "Preferred project duration",
...
const SECTIONS = [
  ...
  [
    "Working Together",
    "working",
    ["duration", "comm", "updates", "approver", "anything_else"],
  ],
];
...
/* ---------------- shared styles ---------------- */
const cardCls =
  "rounded-2xl border border-white/10 bg-[#111318] px-5 py-8 sm:px-10";
...
const SectionHead = ({ no, title, sub }) => (
  <>
    <div className="mb-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-zinc-500">
      {no}
    </div>
    <h2 className="text-xl font-bold text-white sm:text-2xl">{title}</h2>
    {sub && (
      <p className="mb-7 mt-2 text-sm font-light leading-relaxed text-neutral-400">
        {sub}
      </p>
    )}
  </>
...
const Chips = ({ name, options, multi, answers, setField, toggleMulti }) => {
  ...
  const isOn = (v) =>
    multi ? Array.isArray(cur) && cur.includes(v) : cur === v;
  return (
...
const BookPlan = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedDraft.plan && PLANS[savedDraft.plan] && !selectedPlan)
        setSelectedPlan(savedDraft.plan);
      const data = {};
      for (const [k, v] of Object.entries(savedDraft.data || {})) {
        if (k === "_meta" || !(k in LABELS)) continue;
        data[k] = MULTI_FIELDS.includes(k)
          ? Array.isArray(v)
            ? v
            : v
              ? [v]
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      STORE_KEY,
      JSON.stringify({ plan: selectedPlan, data: answers }),
    );
    setSaved(true);
    ...
    fetchJson("/api/rates")
      .then((cats) => {
        ...
        const cat = cats.find((c) =>
          /brand\s*identity/i.test(`${c.id || ""} ${c.label || ""}`),
        );
        ...
        cat.plans.forEach((p) => {
          const k =
            ...
            (String(`${p.name || ""} ${p.id || ""}`)
              .toLowerCase()
              .match(/silver|gold|platinum/) || [])[0];
          if (k && Number(p.price) > 0)
            map[k] = { price: Number(p.price), currency: p.currency || "USD" };
        });
  ...
  const usdOf = (key) => {
    ...
    if (d)
      return d.currency === "USD" ? d.price : Math.round(d.price / fx.rate);
    return planUSD(key);
  };
  const ngnOf = (key) => {
    const d = dbPrices?.[key];
  ...
  const applyCode = async () => {
    ...
    try {
      const r = await fetchJson(
        `/api/discounts/validate?code=${encodeURIComponent(code)}&service=brand&price=${ngnOf(selectedPlan)}`,
      );
      setDiscountInfo(r);
    } catch {
      setDiscountInfo({
  ...
  const effectivePrice = plan
    ? offer && offer.planKey === selectedPlan
      ? offer.price
      : discountInfo?.ok
  ...
  const merchPicked = Array.isArray(answers.merch_designs)
    ? answers.merch_designs
    : [];
  ...
  const toggleMerch = (item) =>
    setAnswers((a) => {
      ...
      if (cur.includes(item))
        return { ...a, merch_designs: cur.filter((x) => x !== item) };
  ...
  const addCustomMerch = () => {
    ...
    if (!item) return;
    if (merchPicked.some((x) => x.toLowerCase() === item.toLowerCase())) {
      setCustomMerch("");
      return;
    }
  ...
  const validateStep = () => {
    ...
    if (step === "about") {
      const missing = ["email", "phone", "brand_name"].filter(
        (n) => !String(answers[n] || "").trim(),
      );
      if (missing.length) {
        flash(missing);
      ...
      <div className="mx-auto max-w-[880px] px-5 pb-20 pt-28 sm:px-6 sm:pt-32">
        ...
        <div className="mb-9 text-center">
          ...
          <p className="mx-auto mt-3 max-w-[580px] text-sm font-light leading-relaxed text-neutral-400">
            Choose the package that fits, then answer a short questionnaire so
            the brand and website we create truly fit your business. Every
            answer is optional except your contact details.
          </p>
          {plan && (
        ...
        <div className="sticky top-0 z-50 mb-2 bg-gradient-to-b from-black from-75% to-transparent pb-5 pt-4">
          ...
          <div className="mt-2.5 flex items-center justify-between text-xs text-neutral-400">
            <span>
              Step <b className="font-semibold text-lime-400">{stepIdx + 1}</b>{" "}
              of <b className="font-semibold text-lime-400">{flow.length}</b> —{" "}
              {STEP_NAMES[step]}
            </span>
            <span className="text-[11px] text-zinc-600">
              Progress saves automatically{" "}
              <span
                className={`text-lime-400 transition-opacity duration-300 ${saved ? "opacity-100" : "opacity-0"}`}
              >
                ✓ saved
              </span>
            </span>
        ...
        {step === "plan" && (
          ...
          >
            <SectionHead
              ...
              sub="Each plan is a complete package — pick the one that fits where your brand is right now."
            />
              ...
              {Object.entries(PLANS).map(([key, p]) => {
                ...
                return (
                  ...
                  >
                    <img
                      src={BADGES[key]}
                      alt={`${p.label} plan badge`}
                      className="h-16 w-auto self-start object-contain"
                    />
                    <div className="mt-3 text-lg font-bold">{p.label}</div>
                    <div className="mt-1 min-h-9 text-xs font-light leading-snug text-neutral-400">
                      {p.blurb}
                    </div>
                    <div className="mt-3 text-2xl font-extrabold text-lime-400">
                      <small className="mr-1 align-top text-[11px] font-semibold text-neutral-400">
                        NGN
                      </small>
                      {price.toLocaleString()}
                    </div>
                    <div className="mt-0.5 text-[11px] text-zinc-500">
                      {DEPOSIT_PCT}% deposit: {N(pct(price, DEPOSIT_PCT))} • $
                      {usdOf(key)}
                    </div>
                    <ul className="mt-3.5 border-t border-white/10 pt-3">
                      {p.deliverables.slice(0, 6).map((d) => (
                        <li
                          key={d}
                          className="relative py-0.5 pl-4 text-xs font-light text-neutral-400"
                        >
                          <span className="absolute left-0 text-[10px] text-lime-400">
                            ✓
                          </span>
                          {d}
                        </li>
        ...
        {step === "about" && (
          <div className={cardCls}>
            <SectionHead
              ...
              sub="The basics — who you are and what your business does."
            />
            <div className="grid gap-x-4 sm:grid-cols-2">
              <Field label="First name">
                <FormInput {...bind} name="first_name" />
              </Field>
              <Field label="Last name">
                <FormInput {...bind} name="last_name" />
              </Field>
            </div>
            <div className="grid gap-x-4 sm:grid-cols-2">
            ...
            <Field label="Name of your brand / business" required>
              <FormInput
                {...bind}
                name="brand_name"
                placeholder="The name your customers will know you by"
              />
            </Field>
            <Field
              label="Tell me about your business and what it offers"
              hint="What services do you provide, and how? The more detail, the better."
            >
              <FormArea {...bind} name="business_about" />
            </Field>
            <Field
              label="What does your business focus on?"
            ...
            <Field label="Which areas / locations do you serve?">
              <FormInput
                {...bind}
                name="service_areas"
                placeholder="e.g. Okota, Lekki, V.I. — or nationwide"
              />
            </Field>
            <Field
              label="Certifications, training or insurance"
              hint="Anything that builds customer trust — business registration, trained staff, insurance cover."
            >
              <FormArea {...bind} name="trust_signals" />
            </Field>
            <Field
              label="What makes your firm different from every other company in your industry?"
              hint="Your method, your guarantee, your story, your standards — this can power a dedicated page on your website."
            >
              <FormArea {...bind} name="peculiarity" />
            </Field>
            <div className="grid gap-x-4 sm:grid-cols-2">
              <Field label="Short-term business goals">
                <FormArea
                  {...bind}
                  name="goals_short"
              ...
              <Field label="Long-term business goals">
                <FormArea
                  {...bind}
                  name="goals_long"
        ...
        {step === "brand" && (
          <div className={cardCls}>
            <SectionHead
              ...
              sub="How your brand should look, feel and be perceived."
            />
            <Field
              label="What's the idea behind your brand's name?"
              hint="The story, meaning, or inspiration behind the name — where it came from and what it stands for."
            >
              <FormArea {...bind} name="name_story" />
            </Field>
            <Field
              label="Who is your ideal target audience?"
              hint="Be specific — e.g. busy professionals, estate residents, offices, property managers."
            >
              <FormArea {...bind} name="audience" />
            </Field>
            <Field label="What are your brand values and mission?">
              <FormArea {...bind} name="values" />
            </Field>
            <Field
              label="Is there a vision you want the brand to paint?"
              hint="Describe the picture you see when you imagine the finished brand and logo — scenes, feelings, imagery, symbols."
            >
              <FormArea {...bind} name="brand_vision" />
            </Field>
            <Field
              label="How do you want customers to perceive your brand?"
              hint="e.g. trustworthy, premium, friendly, professional."
            >
              <FormInput
                {...bind}
                name="perception"
                placeholder="3–5 words that should come to mind"
              />
            </Field>
            <Field
              label="If your brand were a person, how would you describe them?"
              hint="3–5 adjectives — e.g. confident, warm, meticulous, playful."
            >
              <FormInput {...bind} name="personality" />
            </Field>
            <Field
              label="Describe your desired design aesthetic"
              hint="e.g. clean and minimal, bold and modern, premium and elegant."
            >
              <FormInput {...bind} name="aesthetic" />
            </Field>
            <Field
              label="Any colour preferences?"
              hint="Colours you love, colours to avoid, and why."
            >
              <FormInput {...bind} name="colors" />
            </Field>
            <Field label="Do you have an existing logo or visual identity?">
              <Chips
                ...
                options={[
                  ...
                  "Yes — wants a revamp",
                  "Yes — open to fresh direction",
                ]}
              />
            </Field>
            <Field
              label="Brands or designs you admire"
              hint="Any industry. Names or links, and what you like about them."
            >
              <FormArea {...bind} name="inspiration" />
            </Field>
            <Field label="Anything you definitely don't want in your design?">
              <FormInput {...bind} name="avoid" />
            </Field>
            <Field label="Who are your key competitors?">
              <FormInput {...bind} name="competitors" />
            </Field>
            <Field
              label="Do you have a slogan or tagline?"
              hint="If not, I can help craft one."
            >
              <FormInput {...bind} name="tagline" />
            </Field>
              ...
              >
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      !customMerch.trim() || merchPicked.length >= merchMax
                    }
                  >
                    Add
                  </Button>
                ...
                {merchPicked.length >= merchMax && (
                  <p className="mt-2 text-xs text-neutral-500">
                    That's all {merchMax} used — tap a selected item to swap it
                    out.
                  </p>
                )}
                {merchPicked.length > merchMax && (
                  <p className="mt-2 text-xs text-orange-400">
                    Your {plan?.label} package includes {merchMax} — remove{" "}
                    {merchPicked.length - merchMax} to continue.
                  </p>
                )}
              </Field>
            )}
          ...
          <div className={cardCls}>
            <SectionHead
              ...
              sub={
                selectedPlan === "gold" ? (
                  <>
                    Your package includes a 5-page website:{" "}
                    <b className="font-medium text-lime-400">
                      Landing page, About Us, Contact, Portfolio / Previous
                      Projects,
                    </b>{" "}
                    and a{" "}
                    <b className="font-medium text-lime-400">
                    ...
                    built around what makes your firm unique.
                  </>
                ) : (
            ...
            <Field label="What actions should visitors take on the site?">
              <FormInput
                {...bind}
                name="site_actions"
                placeholder="e.g. request a quote, call, book a service"
              />
            </Field>
            <Field
              label="The specialty page — what should it showcase?"
              hint="Building on what makes your firm different: your process, standards, a signature service, your guarantee."
            >
              <FormArea {...bind} name="specialty_page" />
            </Field>
            <Field
              label="Do you have website content ready?"
              hint="Text about your business, service descriptions, team info."
            >
              <Chips
                {...cbind}
                name="content_status"
                options={[
                  "Finalized and ready",
                  "Have some — needs revision",
                  "Needs to be created",
                ]}
              />
            </Field>
            <Field
              label="Do you have photos of past projects?"
              hint="Before/after shots, your team at work — these power the portfolio page."
            >
              <Chips
                {...cbind}
                name="photos"
                options={["Yes — plenty", "A few", "None yet"]}
              />
            </Field>
            <Field
              label="Websites you like"
              hint="Any industry — links and what you like about them."
            >
              <FormArea {...bind} name="site_refs" />
            </Field>
            ...
            <Field
              label="Preferred domain names — in order of preference"
              hint="e.g. yourbrand.com / yourbrand.ng — I'll check availability and advise."
            >
            ...
            <div className="rounded-[10px] border border-lime-400/40 bg-lime-400/10 px-4 py-3.5 text-[13px] font-light leading-relaxed">
              <b className="font-semibold text-lime-400">Please note:</b>{" "}
              website hosting and domain name are billed separately and paid by
              you directly to the providers — all accounts are created with your
              own credentials, so you fully own them. I handle the setup, design
              and development.
            </div>
          </div>
        )}
        ...
        {step === "social" && (
          <div className={cardCls}>
            <SectionHead
              no={secNo("social")}
              title="Social Media"
              sub="Your package includes social media designs — let's make sure they hit the right note."
            />
            <Field
              label="Which platforms are you currently active on?"
              hint="Share handles or links if available."
            >
              <FormInput {...bind} name="social_active" />
            </Field>
            ...
            <Field label="What should the social media designs promote or announce?">
              <FormInput {...bind} name="social_content" />
            </Field>
            <Field
              label="Preferred tone of voice"
              hint="e.g. professional, friendly, premium, playful."
            >
              <FormInput {...bind} name="social_tone" />
            </Field>
            <Field label="Any social media accounts you admire?">
              <FormInput {...bind} name="social_refs" />
            </Field>
          </div>
        )}
        ...
        {step === "working" && (
          <div className={cardCls}>
            <SectionHead
              ...
              sub="How you'd like the project to run, day to day."
            />
            <Field
              label="How long would you like the project to take?"
              hint="Your preference — the final timeline is confirmed with you after I review your answers and scope."
            >
              <Chips
                {...cbind}
                name="duration"
            ...
            <Field label="Preferred communication channels">
              <Chips
                ...
                name="comm"
                multi
                options={["WhatsApp", "Phone calls", "Email", "Video calls"]}
              />
            </Field>
            <Field label="How often would you like progress updates?">
              <Chips
                {...cbind}
                name="updates"
                options={["Daily", "Every 2–3 days", "At each milestone"]}
              />
            </Field>
            <Field label="Who will review work and give approvals on your end?">
              <FormInput {...bind} name="approver" placeholder="e.g. Myself" />
            </Field>
            <Field label="Anything else I should know before we begin?">
              <FormArea {...bind} name="anything_else" />
            </Field>
          </div>
        )}
        ...
        {step === "review" && (
          <div className={cardCls}>
            ...
            <div className="mb-4 overflow-hidden rounded-xl border border-white/10">
              <h3 className="flex justify-between bg-[#0c0e12] px-5 py-3.5 text-sm font-semibold text-lime-400">
                ...
                <button
                  ...
                  className="text-xs font-normal text-neutral-400 hover:text-lime-400"
                >
                  Edit ✎
                </button>
              </h3>
              <div className="px-5 py-3 text-[13px]">
                <div className="flex gap-3.5 py-2">
                  <div className="w-[42%] shrink-0 font-light text-neutral-400">
                    Package
                  </div>
                  <div>
                    {data._meta.plan} — {N(effectivePrice)} ({DEPOSIT_PCT}%
                    deposit:{" "}
                    <span className="text-[#c7b6ff]">
                      {N(pct(effectivePrice, DEPOSIT_PCT))}
                    </span>
                    )
                ...
                {offer ? (
                  <div className="mt-1 rounded-lg border border-lime-400/30 bg-lime-400/5 px-3.5 py-2.5 text-xs text-lime-300">
                    ...
                    {offer.clientName ? ` for ${offer.clientName}` : ""} — your
                    agreed price is {N(offer.price)}.
                  </div>
                ) : (
            ...
            {SECTIONS.filter(([, key]) => key !== "website" || hasWeb).map(
              ([title, key, fields]) => (
                <div
                  key={key}
                  ...
                  <div className="px-5 pb-3.5 pt-1.5">
                    {fields.map((f) => {
                      ...
                      return (
                        <div
                          key={f}
                          className="flex gap-3.5 border-b border-dashed border-white/10 py-2 text-[13px] last:border-b-0"
                        >
                          <div className="w-[42%] shrink-0 font-light text-neutral-400">
                            {LABELS[f]}
                          </div>
                          <div
                            className={`whitespace-pre-wrap break-words ${val ? "" : "italic text-zinc-600"}`}
                          >
                            {val || "Not answered"}
                          </div>
                        </div>
      ...
      {result && (
        ...
        >
          ...
          >
            {selectedPlan && (
              <img
                src={BADGES[selectedPlan]}
                alt={`${plan?.label} plan badge`}
            ...
            <h2 className="text-2xl font-bold">
              Thank you{data.first_name ? `, ${data.first_name}` : ""}!
            </h2>
            ...
            {result.ok && result.invoiceNo && (
              <p className="mt-2 text-sm text-neutral-400">
                Invoice no:{" "}
                <b className="font-semibold text-[#c7b6ff]">
                  {result.invoiceNo}
                </b>
              </p>
            )}
            <div className="my-6 rounded-xl border border-white/10 px-5 py-4 text-left">
              <h4 className="mb-3 text-xs font-semibold uppercase tracking-[0.1em] text-lime-400">
                Next steps
              </h4>
              <ol className="ml-4 list-decimal space-y-2 text-[13px] font-light leading-relaxed">
                {result.ok ? (
                  <>
                    <li>
                      Check your inbox (
                      <b className="font-semibold text-lime-400">
                        {data.email}
                      </b>
                      ) for your{" "}
                      <b className="font-semibold text-lime-400">
                    ...
                    <li>
                      Make the{" "}
                      <b className="font-semibold text-lime-400">
                        {DEPOSIT_PCT}% deposit of {N(data._meta.deposit)}
                      </b>{" "}
                      using the payment details on the invoice.
                    </li>
                    <li>
                      Send your{" "}
                      <b className="font-semibold text-lime-400">
                    ...
                    <li>
                      Once acknowledged, your project begins and I'll confirm
                  ...
                  <>
                    ...
                    <li>
                      You'll receive your{" "}
                      <b className="font-semibold text-lime-400">
                        invoice and terms of agreement
                      </b>{" "}
                      by email.
                    </li>
                    <li>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <Button
                ...
                variant="ghost"
                size="md"
                onClick={() => downloadResponses("txt")}
              >
                Download responses (.txt)
              </Button>
              <Button
                type="button"
                variant="ghost"
            ...
            <div
              className={`mt-3.5 text-xs ${result.ok ? "text-lime-400" : "text-orange-300"}`}
            >
              {result.ok
                ? "✓ Submission received"
                : "The booking service did not respond — use the download button so nothing is lost."}
            </div>
          </div>
        </div>
      )}
