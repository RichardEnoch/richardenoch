...
const DEFAULT_PLANS = {
  ...
  event: {
    label: "Event Campaign (6+ designs)",
    designs: null,
    priceNGN: null,
    perDesign: 9000,
    sourceFiles: true,
...
const F = ({ label, optional, children }) => (
  <Field label={label} hint={optional ? "(optional)" : ""}>
    {children}
  </Field>
);
...
const BookFlyer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEvent && !form.breakdown.trim())
      return setError(
        "List the designs your event needs — a rough list is fine.",
      );
        ...
        {done ? (
          ...
          >
            ...
            <p className="mt-3 mb-10 text-[15px] leading-[1.65] text-white/50 max-w-[520px]">
              Pick a package, drop the details, and get print-and-social-ready
              designs. The more designs you book, the less each one costs.
            </p>
                  ...
                  >
                    ...
                    <ul className="mt-4 space-y-1.5 text-[13px] text-white/55">
                      <li>
                        •{" "}
                        {p.designs
                          ? `${p.designs} design${p.designs > 1 ? "s" : ""}`
                          : "6+ designs — anticipate, countdown, speakers, thank-you & more"}
                      </li>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <F label="When do you need it?" optional>
                  <Input
                    type="date"
                    value={form.deadline}
                ...
                <F label="Headline text" optional>
                  <Input
                    value={form.headline}
                    maxLength={300}
                    onChange={set("headline")}
                    placeholder="The big text on the flyer"
                  />
                </F>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <F label="Subtitle text" optional>
                  <Input
                    value={form.subtitle}
                    maxLength={300}
                ...
                <F label="Date / time / venue" optional>
                  <Input
                    value={form.eventDetails}
                    maxLength={1000}
                    onChange={set("eventDetails")}
                    placeholder="If the flyer needs them"
                  />
                </F>
              </div>
              ...
              {isEvent && (
                <F label="List the designs your event needs *">
                  <Textarea
                    ...
                    maxLength={2000}
                    onChange={set("breakdown")}
                    placeholder={
                      "e.g. main event flyer (2 versions), anticipate design, countdown set, speaker lineup, individual speaker flyers, thank-you design…"
                    }
                  />
                </F>
              )}
                ...
                <p className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-[13px] text-white/55">
                  {selected.priceNGN != null ? (
                    <>
                      ...
                      . Payment details arrive by email after you submit.
                    </>
                  ) : (
                    <>
                      Selected:{" "}
                      ...
                      — quoted per project from {N(selected.perDesign)}/design.
                      I'll reach out within 24 hours with a full quote.
                    </>
                  )}
                </p>
              )}
