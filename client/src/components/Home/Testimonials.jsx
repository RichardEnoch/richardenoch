...
/* ─── Data ───
   ...
   nothing on this site can back up. On a portfolio that asks a reader to trust
   every other number on it, an unverifiable quote costs more than an empty
   space does.

   Rather than delete the section, it renders an honest placeholder. That keeps
   the slot visible — as a standing reminder to go and collect real ones — and
   gives a past client somewhere to click.

   Real quotes go in here, one object per person, in the shape:
     { id, name, title, tag, tagColor, stars, text }
   The scroll-stack comes back automatically the moment this array is filled.
   Anyone can leave one at /testimonial. */
const TESTIMONIALS = [];
...
const restingFor = (n) =>
  Array.from({ length: n }, (_, i) =>
    i === n - 1
      ...
      : {
          y: (n - 1 - i) * 2.25,
          rotate: REST_ROTS[i % REST_ROTS.length],
          x: REST_XS[i % REST_XS.length],
        },
  );
...
const GLASS_ICONS = [
  {
    ...
    dy: [0, -8, 12, -13, 7, -5, 0],
    ddur: 42,
    ps: [1, 1.06, 0.96, 1.05, 0.97, 1.04, 1],
    pdur: 12,
    sdeg: 360,
    sdur: 60,
  },
  {
    ...
    dy: [0, 11, -10, 9, -7, 10, 0],
    ddur: 36,
    ps: [1, 0.95, 1.07, 0.96, 1.06, 0.97, 1],
    pdur: 10,
    sdeg: -360,
    sdur: 50,
  },
  {
    src: "/spiral-glass.png",
    size: 140,
    pos: { bottom: "5%", left: "22%" },
    dx: [0, 8, -11, 6, -9, 5, 0],
    dy: [0, -10, 6, -14, 8, -4, 0],
    ddur: 48,
    ps: [1, 1.05, 0.94, 1.07, 0.96, 1.04, 1],
    pdur: 14,
    sdeg: 360,
    sdur: 70,
  },
  {
    src: "/abstract-glass.png",
    size: 115,
    pos: { top: "18%", right: "10%" },
    dx: [0, -9, 13, -6, 10, -11, 0],
    dy: [0, 12, -7, 11, -10, 6, 0],
    ddur: 38,
    ps: [1, 0.96, 1.06, 0.94, 1.07, 0.97, 1],
    pdur: 11,
    sdeg: -360,
    sdur: 55,
  },
...
const Star = ({ filled }) => (
  <svg
    ...
    height="14"
    viewBox="0 0 24 24"
    fill={filled ? "#84cc16" : "rgba(255,255,255,0.15)"}
  >
    ...
    >
      <div
        style={{
          ...
          height: "100%",
          background:
            "linear-gradient(145deg, rgba(22,22,22,0.97) 0%, rgba(10,10,10,0.99) 100%)",
          border: "1px solid rgba(255,255,255,0.07)",
          borderRadius: 22,
          padding: "32px 40px",
          backdropFilter: "blur(24px)",
          boxShadow:
            "0 28px 70px rgba(0,0,0,0.75), 0 0 0 1px rgba(255,255,255,0.03) inset",
          display: "flex",
...
/* ════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════ */
/* The stack itself. Split out from the export below because every hook in
   here — useScroll in particular — needs a mounted element to measure. An
   early return inside this component would leave useScroll holding a ref that
   never attached, which Framer Motion reports as "Target ref is defined but
   not hydrated" and React Router turns into a full-page error. So the empty
   case is handled one level up, before any of these hooks are called. */
const TestimonialStack = ({ items }) => {
  ...
  const nudgeOpacity = useTransform(
    scrollYProgress,
    [0, 0.04, 0.12],
    [0, 1, 0],
  );
  ...
  return (
    <section
      ...
      style={{ height: `${Math.max(200, items.length * 64)}vh` }}
    >
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col items-center justify-center gap-10">
        ...
        <div
          ...
          style={{
            ...
            height: 500,
            background:
              "radial-gradient(ellipse,rgba(132,204,22,0.04) 0%,transparent 68%)",
            filter: "blur(40px)",
          }}
        />
        ...
        <motion.div
          className="text-center px-6 pointer-events-none relative z-10"
        ...
        >
          ...
          <h2
            className="text-[34px] sm:text-[46px] lg:text-[58px] font-['Outfit'] font-semibold leading-[0.92] tracking-[-0.04em]"
            style={{
...
/* The placeholder. One screen, no scroll-jacking — an empty section has no
   business holding the page hostage the way a full one does. */
const TestimonialsEmpty = () => (
  <section className="relative bg-[#050505] py-24 sm:py-32">
    ...
    <div className="relative z-10 mx-auto flex max-w-[720px] flex-col items-center px-6 text-center">
      <div className="mb-5 inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur">
        <span className="text-[11px] font-semibold uppercase tracking-widest text-neutral-50/80">
          Testimonials
        </span>
      </div>

      <h2
        className="font-['Outfit'] text-[34px] font-semibold leading-[0.92] tracking-[-0.04em] sm:text-[46px] lg:text-[58px]"
        style={{
          background: "linear-gradient(180deg,#ffffff 0%,#7a7a7a 100%)",
          WebkitBackgroundClip: "text",
          WebkitTextFillColor: "transparent",
      ...
      <div className="mt-9 w-full max-w-[560px] rounded-2xl border border-dashed border-white/12 bg-white/[0.02] px-7 py-9">
        <p className="text-[15px] leading-[1.75] text-neutral-400">
          This space is being kept honest. The quotes that were here could not
          all be attributed to a real person, so they came down — and nothing
          goes back up until it is a real client saying a real thing.
        </p>
        <p className="mt-4 text-[15px] leading-[1.75] text-neutral-400">
          If we have worked together, you are the fastest way to fix that.
        </p>
        <Link
          to="/testimonial"
          className="mt-6 inline-block text-[13.5px] font-semibold text-lime-400 hover:underline"
        >
          Leave a testimonial →
        </Link>
      </div>
